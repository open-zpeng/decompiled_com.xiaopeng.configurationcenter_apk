...
public class RemoteOssStorageImpl implements IRemoteStorage {
    ...
    public void uploadWithCallback(IRemoteStorage.CATEGORY category, String str, String str2, Callback callback) throws Exception {
        doPreCheck();
        ThreadUtils.postBackground(createUploadTask(Bucket.get(category)).application(this.mApplication).module(str).filePath(str2).callback(callback).needCertified(this.mCertified).build());
    }
    ...
    public void uploadWithCallback(IRemoteStorage.CATEGORY category, String str, String str2, Callback callback, Map<String, String> map) throws Exception {
        doPreCheck();
        ThreadUtils.postBackground(createUploadTask(Bucket.get(category)).application(this.mApplication).module(str).filePath(str2).callback(callback).needCertified(this.mCertified).remoteCallbackParams(map).build());
    }

    @Override // com.xiaopeng.lib.framework.moduleinterface.netchannelmodule.remotestorage.IRemoteStorage
    public void uploadWithPathAndCallback(String str, String str2, String str3, Callback callback) throws Exception {
        doPreCheck();
        ThreadUtils.postBackground(createUploadTask(new Bucket(str)).application(this.mApplication).remoteFolder(str2).filePath(str3).callback(callback).needCertified(this.mCertified).build());
    }
    ...
    public void uploadWithPathAndCallback(String str, String str2, String str3, Callback callback, Map<String, String> map) throws Exception {
        doPreCheck();
        ThreadUtils.postBackground(createUploadTask(new Bucket(str)).application(this.mApplication).remoteFolder(str2).filePath(str3).callback(callback).needCertified(this.mCertified).remoteCallbackParams(map).build());
    }

    @Override // com.xiaopeng.lib.framework.moduleinterface.netchannelmodule.remotestorage.IRemoteStorage
    public void appendWithPathAndCallback(String str, String str2, byte[] bArr, Callback callback) throws Exception {
        doPreCheck();
        BaseOssTask callback2 = new AppendableTask(new Bucket(str)).application(this.mApplication).remoteFolder(str2).needCertified(this.mCertified).callback(callback);
    ...
    public void downloadWithPathAndCallback(String str, String str2, String str3, Callback callback) throws Exception {
        doPreCheck();
        ThreadUtils.postBackground(new DownloadTask(new Bucket(str)).application(this.mApplication).remoteFolder(str2).filePath(str3).callback(callback).needCertified(this.mCertified).build());
    }
